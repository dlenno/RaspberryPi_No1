...
class Programer
    {
        ...
        private static async Task PostReading(Reading rd)
        {
            client = new HttpClient();
            try
            {
                string resourceAddress = "http://KilmoSensorApi.azurewebsites.net/api/reading";    //http://localhost:24233/
                
                string postBody = JsonSerializer(rd);
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage resp = await client.PostAsync(resourceAddress, new StringContent(postBody, Encoding.UTF8, "application/json"));                     
            }
        ...
        public static string JsonSerializer(Reading objectToSerialize)
        {
